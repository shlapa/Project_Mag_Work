...
const heartsDOM = document.querySelectorAll('.js-heart');
...
heartsDOM.forEach(e => e.onclick = async (event) => {
    ...
    if (target.classList.contains('far')) {
        if (target.getAttribute('id') === 'shop-heart') {
            data = await fetchData('/favorite/shop/add/', target.dataset.shopId, 'POST', 'shopId');
        } else {
            data = await fetchData('/favorite/item/add/', target.dataset.itemId, 'POST', 'itemId');
        }
        if (data.success) {
        ...
        if (target.getAttribute('id') === 'shop-heart') {
            ...
            data = await fetchData('/favorite/item/remove/', target.dataset.itemId, 'DELETE', 'itemId');
        }
        if (data.success) {
